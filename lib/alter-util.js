...
module.exports = function (table_name, attributes, options) {
    ...
    return db.sequelize.query(`select A.COLUMN_NAME,A.DATA_TYPE  from user_tab_columns A where TABLE_NAME='${table_name}'`).then(res => {
        if (res) {
            ...
            if (addList.length) {
                console.log(`【sequelize】get new columns: ${addList.join(', ')}. added right now`);
            }
            var addSql = '';
            if (addList.length) {
                _.each(addList, addItem => {
                    ...
                    if (typeName == 'DATE') {
                        ...
                        addSql += `ADD ${addItem.key} INTEGER ${addItem.defaultValue!=undefined?("DEFAULT '"+addItem.defaultValue+"'"):''} ${addItem.allowNull?'NULL':'NOT NULL'} `
