  ...
  var QueryGenerator = {
    ...
    createTableQuery: function (tableName, attributes, options) {
      ...
      var query = [
          ...
          'BEGIN ',
          '',
          '  EXECUTE IMMEDIATE (\'CREATE TABLE <%= table %> (<%= attributes%>) \'); ',
          '  <%= comment %> ',
          '',
          '  <%= sequence %> ',
          '',
      ...
      for (var attr in attributes) {
        if (attributes.hasOwnProperty(attr)) {
          ...
          if (dataType.indexOf(" COMMENT ") > -1) {
            comments.push(`  EXECUTE IMMEDIATE ('COMMENT ON COLUMN ${tableName}.${attr} IS ''${dataType.split(" COMMENT ")[1]}'' '); `)
      ...
      if (!!options.comment) {
        comments.push(`  EXECUTE IMMEDIATE ('COMMENT ON TABLE ${tableName} IS ''${options.comment}'' '); `)
      }
      ...
      var values = {
          ...
          comment: comments.join('\n'),// options.comment && Utils._.isString(options.comment) ? ' COMMENT ' + this.escape(options.comment) : '',
          sequence: sequences,
          trigger: triggers
    ...
    attributeToSQL: function (attribute) {
      ...
      template += !!attribute.description ? ` COMMENT ${attribute.description}` : '';
      return template;
    },
